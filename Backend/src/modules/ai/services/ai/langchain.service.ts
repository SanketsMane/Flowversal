...
export class LangChainService {
  ...
  async createModelWithAccuracyCheck(options: LangChainOptions = {}): Promise<{
    ...
    if (aiConfig.vllm.enabled) {
      try {
        ...
        console.log(`vLLM accuracy ${accuracy.toFixed(2)} below threshold ${accuracyThreshold}, trying OpenRouter...`);
      } catch (error) {
  ...
  async chatCompletion(
    ...
    if (enableAccuracyCheck) {
      ...
      console.log(`🤖 Using ${selectedProvider} model${accuracy ? ` (accuracy: ${(accuracy * 100).toFixed(1)}%)` : ''}`);
    } else {
  ...
  async generateText(
    ...
    if (enableAccuracyCheck) {
      const result = await this.createModelWithAccuracyCheck(options);
      model = result.model;
      console.log(`📝 Using ${result.provider} model for text generation${result.accuracy ? ` (accuracy: ${(result.accuracy * 100).toFixed(1)}%)` : ''}`);
    } else {
      model = this.createModel(options);
    }
