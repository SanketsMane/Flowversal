...
/**
 * Verify user authorization - Production secure
 * Author: Sanket - No demo/test users
 */
async function verifyUser(authHeader: string | null) {
  if (!authHeader) {
    console.error('[Subscription API] Missing authorization header');
    return { error: 'Unauthorized - missing auth header', status: 401 };
  }
  
  const accessToken = authHeader.split(' ')[1];
  if (!accessToken) {
    console.error('[Subscription API] Invalid authorization format');
  ...
  try {
    ...
    if (error || !user) {
      console.error('[Subscription API] Invalid or expired JWT:', error?.message);
      return { error: 'Unauthorized - invalid token', status: 401 };
    }
    
    return { user };
  } catch (err) {
    console.error('[Subscription API] Authentication error:', err);
