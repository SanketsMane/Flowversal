...
// Author: Sanket - No demo users, strict auth only
async function verifyAuth(authHeader: string | undefined) {
  if (!authHeader) {
    console.error('[Workflows API] Missing authorization header');
    return { error: 'Unauthorized - missing auth header', status: 401 };
  }
  
  const accessToken = authHeader.split(' ')[1];
  if (!accessToken) {
    console.error('[Workflows API] Invalid authorization format');
  ...
  try {
    ...
    if (error || !user) {
      console.error('[Workflows API] Invalid or expired JWT:', error?.message);
      return { error: 'Unauthorized - invalid token', status: 401 };
    }
    
    return { user };
  } catch (err) {
    console.error('[Workflows API] Authentication error:', err);
