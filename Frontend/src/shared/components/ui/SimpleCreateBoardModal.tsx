/**
 * Simple Create Board Modal - Compact Design
 */
...
interface SimpleCreateBoardModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (board: { name: string; icon: string; iconColor: string }) => void;
}
...
export function SimpleCreateBoardModal({ isOpen, onClose, onSave }: SimpleCreateBoardModalProps) {
      ...
      >
        ...
        <div className={`p-4 border-b ${borderColor} flex items-center justify-between`}>
          <div className="flex items-center gap-2">
            <LayoutGrid className="w-5 h-5 text-[#00C6FF]" />
            <h2 className={`text-lg ${textPrimary}`}>Create New Board</h2>
          </div>
        ...
        <div className="p-4 space-y-4 max-h-[60vh] overflow-y-auto">
          ...
          <div className={`${bgCard} rounded-lg border ${borderColor} p-3 flex items-center gap-3`}>
            ...
            <div className="flex-1">
              <p className={`text-sm ${textPrimary}`}>{formData.name || 'Board Name'}</p>
              <p className={`text-xs ${textSecondary}`}>0 tasks</p>
            </div>
          </div>
        </div>
